...
func SampleJSONRows(path string, maxRows int) ([]map[string]string, error) {
	...
	if strings.HasSuffix(path, ".jsonl") || (!strings.HasPrefix(trim, "[") && strings.Contains(trim, "\n")) {
		return sampleJSONL(path, maxRows)
	}
	var arr []map[string]any
	if err := json.Unmarshal(data, &arr); err != nil {
		return nil, fmt.Errorf("parse json array: %w", err)
	}
	return mapsToStringRows(arr, maxRows), nil
}

func sampleJSONL(path string, maxRows int) ([]map[string]string, error) {
	f, err := os.Open(path)
	if err != nil {
		return nil, err
	}
	...
	return mapsToStringRows(arr, maxRows), sc.Err()
}
